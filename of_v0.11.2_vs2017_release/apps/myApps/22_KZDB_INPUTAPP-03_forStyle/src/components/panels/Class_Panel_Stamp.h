...
class Class_StampPanel:public Class_Panel {
protected:
...
public:
	...
	glm::vec2 selectedAreaPos;

	Class_StampPanel(){}
	Class_StampPanel(const glm::vec2& _parentGlobalPos, const glm::vec2& _panelPos, const glm::vec2 _panelSize, ofImage* _img) {
	...
	void drawStampedCircle() {
		...
		ofDrawCircle(getDefaultFormatPos(selectedAreaPos), 20);
		glColor4f(1, 1, 1, 1);
		ofNoFill();
		glColor3f(1, 0, 1);
		ofDrawCircle(getDefaultFormatPos(selectedAreaPos), 20);
	...
	void onMouseClicked() {
		if (glm::compMin(customFormatAreaPos) >= -1 && glm::compMax(customFormatAreaPos) <= 1) {
			selectedAreaPos = customFormatAreaPos;
		}
	};
